    ...
    <div class="row justify-content-center">
        <div class="col-lg-12">
            <div class="building-card p-4 p-md-5">
                <header class="mb-4">
                    <h2 class="h4 fw-bold mb-1">Edit Building</h2>
                    <p class="text-muted small">Update the information for this building.</p>
                </header>

                <form action="{{ route('admin.building.update', $building->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                name="name" value="{{ old('name', $building->name) }}" placeholder="Official Building Name">
                            @error('name')
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-select @error('building_type') is-invalid @enderror" id="building_type"
                                ...
                                <option value="{{ $type }}" {{ old('building_type', $building->building_type) == $type ? 'selected' : '' }}>{{ $type }}</option>
                                @endforeach
                            </select>
                            @error('building_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-8">
                            ...
                            <input type="text" class="form-control @error('address') is-invalid @enderror" id="address"
                                name="address" value="{{ old('address', $building->address) }}">
                            @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control @error('city') is-invalid @enderror" id="city"
                                name="city" value="{{ old('city', $building->city) }}">
                            @error('city')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control @error('amenities') is-invalid @enderror" id="amenities"
                                ...
                                placeholder="List key features">{{ old('amenities', $building->amenities) }}</textarea>
                            @error('amenities')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <div class="image-upload-wrapper @error('images*') border-danger @enderror">
                                ...
                                <div id="imagePreview" class="d-flex flex-wrap gap-2 mt-3 justify-content-center">
                                    @if($building->images)
                                        @foreach(json_decode($building->images) as $image)
                                        <div class="position-relative">
                                            <img src="{{ asset('storage/' . $image) }}" alt="Building Image" class="img-thumbnail" style="width: 100px; height: 100px; object-fit: cover;">
                                        </div>
                                        @endforeach
                                    @endif
                                </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status">
                                <option value="1" {{ old('status', $building->status) == '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('status', $building->status) == '0' ? 'selected' : '' }}>Under Maintenance</option>
                            </select>
                            @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-8 d-flex align-items-end justify-content-end gap-3">
                            <a href="{{ route('admin.building.index') }}" class="btn btn-link text-decoration-none text-muted">Cancel</a>
